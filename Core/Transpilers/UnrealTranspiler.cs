...
public class UnrealTranspiler : AbstractTranspiler
{
    public static void Generate()
    {
        ...
        string publicDir = Path.Combine(projectDirectory, "Unreal", "Source", "ToS_Network", "Public", "Packets");
        ...
        GenerateEnum(publicDir, "ClientPacket", clientPackets);
        GenerateEnum(publicDir, "ServerPacket", serverPackets);
    }

    private static void GenerateEnum(string directory, string enumName, List<string> values)
    {
        string filePath = Path.Combine(directory, $"{enumName}.h");
    ...
    {
        string filePath = Path.Combine(directory, $"{structName}.h");
        using var writer = new StreamWriter(filePath);
